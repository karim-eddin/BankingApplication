...
public class Account {
    ...
    void deposit(int amount) {
        if (amount != 0) {
            ...
            if (amount > 500) {
                double fee = amount * 0.005; // Calculate 0.5% fee for deposit
                netAmount = (int) (amount - fee); // Calculate the net amount after the fee is deducted
                System.out.printf("Deposited: %d (net amount after 0.5%% fee: %d)\n", amount, netAmount);
            } else {
                System.out.println("Deposited: " + amount);
            }
            balance = balance + netAmount;
            previousTransaction = netAmount;
        }
    }
